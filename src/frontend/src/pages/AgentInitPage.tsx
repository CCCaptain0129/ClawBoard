...
第一次进入时，请按这个顺序工作：
1. 先读取全局真源规则：GET /api/tasks/source-of-truth
2. 再读取项目列表：GET /api/tasks/projects
3. 进入目标项目后读取真源信息：GET /api/tasks/projects/:projectId/source-of-truth
4. 默认使用 API 作为对外真源，不要假设你能访问看板服务器上的本地目录
5. 如用户正在开启新项目，先创建项目（POST /api/tasks/projects），再建立项目规划、任务分解、进度跟踪三类文档
6. 将拆解后的任务通过后端 API 写入看板（由后端落盘到服务端 tasks/*.json）
7. 在执行过程中，持续通过 API 更新任务状态/负责人（本质是更新服务端 tasks/*.json）
...
真源规则：
- 对外（给 Agent）真源是 API
- 对内（看板服务端）真源是 tasks/*.json
- 前端只是可视化入口
- 项目详细内容保留在项目文件夹和文档中
- MEMORY.MD 只保存长期规则、项目索引和最新进度摘要
- 只有当 Agent 与看板在同一文件系统时，才可使用 source-of-truth 返回的绝对路径
...
const QUICK_START_STEPS = [
  '先读取全局真源规则（/api/tasks/source-of-truth），确认使用 API-first 模式。',
  '读取项目列表，再读取项目级 source-of-truth。',
  '如果是新项目，先创建项目，再建立项目规划、任务分解和进度跟踪文档。',
  '把拆解后的任务通过 API 正式添加到看板（后端会写入服务端 tasks/*.json）。',
  '在执行过程中通过 API 更新任务状态和负责人（不是改前端页面文本）。',
]
...
const API_GUIDE = [
  '读取全局真源规则：GET /api/tasks/source-of-truth',
  '读取项目列表：GET /api/tasks/projects',
  '读取项目任务：GET /api/tasks/projects/:projectId/tasks',
  '读取真源路径：GET /api/tasks/projects/:projectId/source-of-truth',
  ...
  '更新路径映射：PUT /api/tasks/projects/:projectId/source-of-truth',
]

const SOURCE_RULES = [
  '前端页面不是状态真源，只是可视化入口。',
  '外部 Agent 默认使用 API 作为真源，不要求知道可视化项目目录。',
  'tasks/*.json 是服务端内部真源，不是跨环境 Agent 的直接读写入口。',
  '“同步到看板”= 调用后端 API，后端统一写入 tasks/*.json。',
  '新项目必须先通过 API 注册，再维护任务与文档。',
