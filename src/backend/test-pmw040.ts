...
import { MarkdownToJSON } from '../src/sync/markdownToJSON';
...
async function test() {
  ...
  // 写入临时测试文件
  const testFilePath = '/tmp/test-pmw040.md';
  fs.writeFileSync(testFilePath, testMarkdown, 'utf-8');

  try {
    // 创建解析器实例（但我们需要修改它来读取临时文件）
    const parser = new MarkdownToJSON();

    // 由于 MarkdownToJSON 类硬编码了路径，我们通过其他方式测试
    // 这里我们直接测试 parseTask 方法
