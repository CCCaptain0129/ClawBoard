...
const { MarkdownToJSON } = require('./dist/sync/markdownToJSON.js');
...
async function test() {
  ...
  try {
    ...
    // 测试1: 带 ### 格式的任务（从表单创建）
    const task1Line = lines.find(l => l.includes('PMW-001')) || '';
    ...
    const task2Line = lines.find(l => l.includes('PMW-002')) || '';
    ...
    // 测试3: 临时任务区块
    const task3Line = lines.find(l => l.includes('PMW-003')) || '';
